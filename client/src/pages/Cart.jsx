...
const Cart = () => {
    ...
    const {getCartCount,getCartAmount,currency,updateCartItem,products
        ,navigate,removeFromCart, CartItems} = useAppContext();
              ...
              {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div onClick={()=>{
                                navigte(`/products/${product.category.toLowerCase()}/${product._id}`); scrollTo(0,0)
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {Address.map((adress,index)=>(
                                     <p onClick={() => {setSelectedAddress(adress),setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
